...
export const IconToggle = ({ icons, inView }: ToggleSchemaProps) => {
  return (
    <div className="flex w-full  mt-6 pb-2 flex-col">
      <label className="relative flex cursor-pointer flex-nowrap flex-row items-center gap-2 text-slate-500">
        <input
          type="checkbox"
          readOnly
          checked={inView}
      ...
      <span
        className={` ${inView ? " duration-1000 transition-opacity ease-in-out  opacity-100" : " "} mt-2 text-black opacity-0  text-sm dark:text-white text-start`}
      >
        {icons.summary}
      </span>
    </div>
