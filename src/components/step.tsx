...
export const Step = ({ step, index }: StepProps) => {
  ...
  return (
    ...
    >
      ...
      >
        <span className="flex flex-nowrap justify-start gap-3 items-center flex-row">
          <span
            className={`${index % 2 === 0 ? "" : "md:order-2 "}  ml-2 md:ml-0 h-8 w-8 border-[2px] flex self-center text-center items-center rounded-full justify-center align-middle border-rose-600 dark:border-rose-400 text-rose-600 dark:text-rose-400`}
          >
            {index + 1}
          </span>
          <H3Title title={step.title} />
        </span>
        <p className="text-black px-2 md:px-0 my-1 w-full  md:max-w-[66%] dark:text-white text-sm md:text-base  ">
          {step.summaryText}
        </p>
      </div>
      <div
      ...
      >
        <img
          src={step.img}
          className={`object-contain max-h-[200px] md:max-h-[300px] my-4 z-10`}
          alt={step.title}
        />
        <div
